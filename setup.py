...
if sys.version_info < (3, 7):
    print('Python versions prior to 3.7 are not supported for JCP.',
          file=sys.stderr)
    sys.exit(-1)

this_directory = os.path.abspath(os.path.dirname(__file__))
version_file = os.path.join(this_directory, 'src/main/python/jcp/version.py')

try:
    exec (open(version_file).read())
except IOError:
    print("Failed to load JCP version file for packaging. {0} not found!".format(version_file),
          file=sys.stderr)
    sys.exit(-1)
...
extensions = ([
    Extension(
        name="jcp_core",
        sources=get_files('src/main/c/jcp/core', '.c'),
        libraries=get_python_libs(),
        extra_link_args=get_java_linker_args(),
        include_dirs=get_java_include() + ['src/main/c/jcp/core/include'] + get_numpy_include(),
        language=3),
    Extension(
        name="jcp_utils",
        sources=get_files('src/main/c/jcp/utils', '.c'),
        extra_link_args=get_java_linker_args(),
        include_dirs=get_java_include() + ['src/main/c/jcp/utils/include'],
        language=3)
])
