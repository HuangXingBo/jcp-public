...
function tox_check() {
    ...
    if [[ ${BUILD_REASON} = 'IndividualCI' ]]; then
        ...
        $TOX_PATH -c $JCP_DIR/tox.ini -e ${LATEST_PYTHON} --recreate 2>&1 | tee -a $LOG_FILE
    else
        $TOX_PATH -c $JCP_DIR/tox.ini --recreate 2>&1 | tee -a $LOG_FILE
    fi
