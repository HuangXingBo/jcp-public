...
static PyObject* JcpMainThreadModules = NULL;
static PyObject* JcpMainThreadModulesLock = NULL;
...
JcpPy_Initialize(JNIEnv *env)
{
    ...
    /*
     * Save a global reference to the sys.modules form the main thread to support shared modules
     */
    sys_module = PyImport_ImportModule("sys");
    if (sys_module == NULL) {
        (*env)->ThrowNew(env, JILLEGAL_STATE_EXEC_TYPE,
                        "Failed to import sys module in initialization.");
        return;
    }

    JcpMainThreadModules = PyObject_GetAttrString(sys_module, "modules");
    Py_DECREF(sys_module);

    if (JcpMainThreadModules == NULL) {
        (*env)->ThrowNew(env, JILLEGAL_STATE_EXEC_TYPE,
                        "Failed to get `sys.modules` in initialization.");
        return;
    }
    ...
    if (threading_module == NULL) {
        (*env)->ThrowNew(env, JILLEGAL_STATE_EXEC_TYPE,
                        "Failed to import `threading` module in initialization.");
        return;
    }

    lock_creator = PyObject_GetAttrString(threading_module, "Lock");
    Py_DECREF(threading_module);
    if (lock_creator == NULL) {
        (*env)->ThrowNew(env, JILLEGAL_STATE_EXEC_TYPE,
                        "Failed to get `Lock` attribute of `threading` module in initialization.");
        return;
    }

    JcpMainThreadModulesLock = PyObject_CallObject(lock_creator, NULL);
    Py_DECREF(lock_creator);

    if (JcpMainThreadModulesLock == NULL) {
        (*env)->ThrowNew(env, JILLEGAL_STATE_EXEC_TYPE,
                        "Failed to get main thread modules lock.");
        return;
    }
...
intptr_t
JcpPy_InitThread(JNIEnv *env)
{

    JcpThread *jcp_thread;

    PyObject  *tdict, *globals, *key, *t;
    ...
    jcp_thread->env = env;
    jcp_thread->callable_name = NULL;
    jcp_thread->callable = NULL;
...
JcpPy_FinalizeThread(intptr_t ptr)
{
    ...
    if (jcp_thread->callable_name != NULL) {
        free(jcp_thread->callable_name);
    }

    Py_XDECREF(jcp_thread->callable);
...
JcpPy_ImportModule(JNIEnv *env, jstring name)
{
    ...
    if (module) {
        ...
        memset(msg, '\0', 200);
        sprintf(msg, "Failed to import `%s` module.", module_name);
        JcpPyErr_Throw(env);
        (*env)->ThrowNew(env, JILLEGAL_STATE_EXEC_TYPE, (const char*) msg);
    }
                       ...
                       const char *name, jobjectArray args)
{
    ...
    callable = PyObject_GetAttr(py_obj, py_name);
    Py_DECREF(py_name);

    if (callable == NULL) {
        (*env)->ThrowNew(env, JPYTHONEXCE_TYPE, "Failed to find the method");
        goto exit;
    }
...
void
JcpEval(JNIEnv *env, intptr_t ptr, const char *code)
{
    ...
    result = PyRun_String(code, Py_single_input, jcp_thread->globals, jcp_thread->globals);
...
jint
JcpCompile(JNIEnv *env, intptr_t ptr, const char *code)
{
    ...
    result = Py_CompileString(code, "<stdin>", Py_single_input);
