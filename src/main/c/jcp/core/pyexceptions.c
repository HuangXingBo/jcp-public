    ...
    if (type) {
        ...
        if (traceback) {
            ...
            if (traceback_module != NULL && extract_method != NULL) {
                pystack = PyObject_CallMethodObjArgs(traceback_module, extract_method, traceback, NULL);
            }
        ...
        if (pystack != NULL) {
            ...
            jobjectArray stack_elements = (*env)->NewObjectArray(env, stackSize, JSTACK_TRACE_ELEMENT_TYPE, NULL);
            ...
            for (int i = 0; i < stackSize; i++) {
                ...
                if (frame_line != Py_None) {
                    ...
                    jstring jframe_filename_no_dir = (*env)->NewStringUTF(env, (const char *) frame_filename_no_dir);

                    jstring jframe_file_no_suffix = (*env)->NewStringUTF(env, (const char *) frame_filename_no_suffix);

                    jstring jframe_func_name = (*env)->NewStringUTF(env, frame_func_name);

                    jobject stack_trace_element = JavaStackTraceElement_New(env, jframe_file_no_suffix,
                                                    jframe_func_name, jframe_filename_no_dir,
                                                    frame_lineno);
                    ...
                    (*env)->SetObjectArrayElement(env, stack_elements, (jsize) i, stack_trace_element);
                    free(frame_filename_no_suffix);
                    free(frame_filename_no_dir);
            ...
            jobjectArray reverse_jstack = (*env)->NewObjectArray(env, (jsize) (stackSize + jstack_length),
            ...
            for (int i = 0; i < jstack_length; i++) {
                ...
                if (stack_trace_element != NULL) {
                    (*env)->SetObjectArrayElement(env, reverse_jstack, i + stackSize, stack_trace_element);
                    (*env)->DeleteLocalRef(env, stack_trace_element);
                }
            }
