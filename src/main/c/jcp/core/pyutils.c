...
JcpPyObject_FromJObject(JNIEnv* env, jobject value)
{
    ...
    if ((*env)->IsSameObject(env, clazz, JSTRING_TYPE)) {
        ...
        (*env)->ThrowNew(env, JPYTHONEXCE_TYPE, "Unsupported java object type");
    }
...
JcpPyObject_AsJObject(JNIEnv* env, PyObject* pyobject, jclass clazz)
{
    ...
    if (pyobject == Py_None) {
        ...
        return JcpPyString_AsJObject(env, pyobject, clazz);
...
JcpPyString_AsJObject(JNIEnv* env, PyObject* pyobject, jclass clazz)
{
    ...
    } else if ((*env)->IsAssignableFrom(env, JCHAR_OBJ_TYPE, clazz)) {
        c = JcpPyString_AsJChar(pyobject);

        if (c == 0 && PyErr_Occurred()) {
            return NULL;
        }

        result = JavaCharacter_New(env, c);
    }
