...
public class CommonUtils {
    ...
    private static final String GET_JCP_MODULE_PATH_SCRIPT =
            "import jcp;" + "import os;" + "print(os.path.dirname(jcp.__file__))";
    ...
    public void loadLibrary(String pythonExec, String library) {
        if (!initialized) {
            String utilsLibPath =
                    getLibraryPathWithPattern(pythonExec, "^jcp_utils\\.cpython-.*\\.so$");
        ...
        if (pythonExec == null) {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Failed to get jcp path. You need to `pip install jcp` firstly.", e);
            }
            ...
            throw new RuntimeException("Failed to find jcp Library");
        }
    }

    public String getJcpModulePath(String pythonExec) {
        if (pythonExec == null) {
            // run in source code
            return String.join(
                    File.separator, System.getProperty("user.dir"), "src", "main", "python", "jcp");
        } else {
            String out;
            try {
                out = execute(new String[] {pythonExec, "-c", GET_JCP_MODULE_PATH_SCRIPT});
            } catch (IOException e) {
                throw new RuntimeException("Failed to get Jcp module path", e);
            }
    ...
    public String getPythonLibrary(String pythonExec) {
        ...
        String libPathPatter;
        if (isLinuxOs()) {
            libPathPatter = "^libpython.*so$";
        } else if (isMacOs()) {
            libPathPatter = "^libpython.*dylib$";
        } else {
            throw new RuntimeException("Unsupported os ");
        }
        if (libFile.isDirectory()) {
            for (File f : Objects.requireNonNull(libFile.listFiles())) {
                if (f.isFile() && Pattern.matches(libPathPatter, f.getName())) {
                    return f.getAbsolutePath();
                }
            }
        }
        throw new RuntimeException("Failed to find libpython dylib");
    }
