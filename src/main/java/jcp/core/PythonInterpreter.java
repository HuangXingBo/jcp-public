...
public final class PythonInterpreter implements Interpreter {
    ...
    private MainInterpreter mainInterpreter = MainInterpreter.instance;
    ...
    public void eval(String expression) {
        isRunning();
        if (compile(tState, expression) == 1) {
            eval(tState, expression);
        }
    }
    ...
    /**
     * Compiles the source into a code.
     *
     * @param tState the JcpThread
     * @param code the python code
     */
    private native int compile(long tState, String code);
     ...
     * @param code the python code
     */
    private native void eval(long tState, String code);
     ...
     *
     * <p>While JCP does not use those methods, CPython extensions such as numpy do. The deadlock
    ...
    private static class MainInterpreter implements Serializable, AutoCloseable {
        ...
        private static MainInterpreter instance = new MainInterpreter();
         ...
         * Initializes CPython.
         *
         * <p>We load on a separate thread to try and avoid GIL issues that come about from a being
         * on the same thread as the main interpreter.
         */
        synchronized void initialize(String pythonExec) {
            if (!isStarted) {
                String jcpLibPath =
                        CommonUtils.INSTANCE.getLibraryPathWithPattern(
                                pythonExec, "^jcp_core\\.cpython-.*\\.so$");
                String pythonLibPath = CommonUtils.INSTANCE.getPythonLibrary(pythonExec);
                String jcpModulePath = CommonUtils.INSTANCE.getJcpModulePath(pythonExec);
                ...
                System.load(jcpLibPath);
                thread =
                        new Thread("JcpMainInterpreter") {
                            @Override
                            public void run() {
                                try {
                                    initialize();
